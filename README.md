...
- 🎯 Entry points to execute your program using `python -m <fastapi_backend>` or `$ fastapi_backend` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/gparpinelli/fastapi-backend/branch/main/graph/badge.svg?token=fastapi-backend_token_here)](https://codecov.io/gh/gparpinelli/fastapi-backend)
[![CI](https://github.com/gparpinelli/fastapi-backend/actions/workflows/main.yml/badge.svg)](https://github.com/gparpinelli/fastapi-backend/actions/workflows/main.yml)

Awesome fastapi_backend created by gparpinelli

## Install

from source
```bash
git clone https://github.com/gparpinelli/fastapi-backend fastapi_backend
cd fastapi_backend
make install
```
...
```bash
pip install fastapi_backend
```

## Executing

```bash
$ fastapi_backend run --port 8080
```

or

```bash
python -m fastapi_backend run --port 8080
```
...
```bash
❯ fastapi_backend --help
Usage: fastapi_backend [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastapi_backend create-user --help
Usage: fastapi_backend create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastapi_backend create-user admin admin --superuser
```
...
Run with `fastapi_backend run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastapi_backend create-user`.
...
-----------------------------------------------------
fastapi_backend/__init__.py              4      0   100%
fastapi_backend/app.py                  16      1    94%
fastapi_backend/cli.py                  21      0   100%
fastapi_backend/config.py                5      0   100%
fastapi_backend/db.py                   10      0   100%
fastapi_backend/models/__init__.py       0      0   100%
fastapi_backend/models/content.py       47      1    98%
fastapi_backend/routes/__init__.py      11      0   100%
fastapi_backend/routes/content.py       52     25    52%
fastapi_backend/routes/security.py      15      1    93%
fastapi_backend/routes/user.py          52     26    50%
fastapi_backend/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastapi_backend/default.toml.
