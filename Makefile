...
test: lint        ## Run tests and generate coverage report.
	$(ENV_PREFIX)pytest -v --cov-config .coveragerc --cov=fastapi_backend -l --tb=short --maxfail=1 tests/
...
release:          ## Create a new tag for release.
	...
	@git tag $${TAG}
	@echo "$${TAG}" > fastapi_backend/VERSION
	@$(ENV_PREFIX)gitchangelog > HISTORY.md
	@git add fastapi_backend/VERSION HISTORY.md
...
shell:            ## Open a shell in the project.
	...
	@./.venv/bin/ipython -c "from fastapi_backend import *"

.PHONY: docker-build
docker-build:	  ## Builder docker images
	@docker-compose -f docker-compose-dev.yaml -p fastapi_backend build

.PHONY: docker-run
docker-run:  	  ## Run docker development images
	@docker-compose -f docker-compose-dev.yaml -p fastapi_backend up -d

.PHONY: docker-stop
docker-stop: 	  ## Bring down docker dev environment
	@docker-compose -f docker-compose-dev.yaml -p fastapi_backend down

.PHONY: docker-ps
docker-ps: 	  ## Bring down docker dev environment
	@docker-compose -f docker-compose-dev.yaml -p fastapi_backend ps
